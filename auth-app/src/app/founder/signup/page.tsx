...
import { Github, Loader2, ArrowRight, ShieldCheck } from 'lucide-react';
...
export default function FounderSignupPage() {
    ...
    const handleSignUp = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        const { error } = await signUp(email, password, 'founder');
            ...
            <div className="w-full max-w-[440px] space-y-8">
                ...
                <div className="space-y-2 text-center">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/10 border border-accent/20 text-[10px] font-medium uppercase tracking-wider text-accent mb-2">
                        <ShieldCheck className="w-3 h-3" /> Founder Access
                    </div>
                    <h1 className="text-2xl font-semibold tracking-tight">Create Founder Account</h1>
                    <p className="text-sm text-muted">Join the community to build and lead your project</p>
                </div>
                ...
                <p className="text-center text-xs text-muted">
                    Already lead a project? <Link href="/founder/login" className="text-accent hover:underline font-medium">Log in</Link>
                </p>

                <div className="pt-8 text-center">
                    <Link href="/builder/signup" className="text-xs text-muted hover:text-foreground transition-colors border-b border-border/50 pb-0.5">
                        Are you a Builder? Join as Builder
                    </Link>
                </div>
            </div>
        </div>
